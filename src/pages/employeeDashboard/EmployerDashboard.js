...
const EmployerDashboard = () => {
  ...
  const { data, isLoading } = useGetJobByEmailQuery(user.email);
  ...
  const handleDelete = (id) => {
    swal({
      ...
      if (willDelete) {
        deleteJob(id);
        swal("Successful! Your job has been deleted!", {
          icon: "success",
        });
      } else {
        swal("Your Jobs is safe!");
      }
    });
  };
  return (
        ...
        <div className="container py-2 mx-auto sm:p-4 text-primary">
          {data?.data?.length ? (
            <h1 className="w-full text-2xl text-primary mb-5">
              Open To Jobs - {data?.data?.length}
            </h1>
          ) : (
            <h1 className="w-full text-2xl text-primary mb-5">
              No Post Jobs Available, Please Add Jobs!
            </h1>
          )}
